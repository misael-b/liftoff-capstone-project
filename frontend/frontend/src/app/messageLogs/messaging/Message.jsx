            ...
            <div className="messageLogArea">
                {
                        data.map((item) => {
                            if (item.user.username === fetchedUser) {
                                return (<div><p id="1">{item.message}</p><Link href="http://localhost:3000/messageLogs/messaging/update">Update</Link><Link href="http://localhost:3000/messageLogs/messaging/delete">Delete</Link></div>)
                            } else if (item.user.username === fetchedOtherUser) {
                                return (<div><p id="2">{item.message}</p><Link href="http://localhost:3000/messageLogs/messaging/update+">Update</Link><Link href="http://localhost:3000/messageLogs/messaging/delete">Delete</Link></div>)
                            } else {
                                return (<p>help</p>)
                            }
                        })
